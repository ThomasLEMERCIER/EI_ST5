...
def soft_DirectGradientDescent(chi0, domain_omega, spacestep, wavenumber, Alpha, K, lr=1):
    chi = np.copy(chi0)
    ...
    for k in range(K): 
        ...
        grad_J = utils.compute_grad_J_euler_soft(chi, beta, domain_omega, spacestep, wavenumber, Alpha)

        chi = chi - lr * grad_J
        chi = utils.softmax_project(chi, beta, domain_omega)
...
def soft_DirectGradientDescent_Adam(chi0, domain_omega, spacestep, wavenumber, Alpha, K, lr=1):
